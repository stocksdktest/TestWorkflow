...
class MongoHookWithDB(BaseHook):
    """
    ...
    def __init__(self, conn_id='mongo_default', *args, **kwargs):
        super(MongoHookWithDB, self).__init__(source='mongo')

        self.mongo_conn_id = conn_id
        self.connection = self.get_connection(conn_id)
        self.extras = self.connection.extra_dejson.copy()
        self.database = self.extras.get('database', self.connection.schema)
        self.client = None

        srv = self.extras.pop('srv', False)
        scheme = 'mongodb+srv' if srv else 'mongodb'

        self.uri = '{scheme}://{creds}{host}{port}'.format(
            scheme=scheme,
            creds='{}:{}@'.format(
    ...
    def get_collection(self, mongo_collection, mongo_db=None):
        ...
        """
        mongo_db = mongo_db if mongo_db is not None else self.database
