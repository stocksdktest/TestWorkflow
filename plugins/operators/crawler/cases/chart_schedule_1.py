        ...
        if self.TYPE == 'ChartTypeBeforeData':
            for cr in crawler_result:
                dictionary[re.sub('[- :]','',cr['transactionTime']) if 'transactionTime' in cr.keys() else 'isEmpty'] = {
                    ...
                    'closePrice' : cr['transactionPrice'] if 'transactionPrice' in cr.keys() else '-',
                    # 'tradeVolume' : cr['singleVolume'] if 'singleVolume' in cr.keys() else '-',
                    # 'averagePrice' : cr['averagePrice'] if 'averagePrice' in cr.keys() else '-',
                }
